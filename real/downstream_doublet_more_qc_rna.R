...
B = vale_clean$FRiP[ colnames(vale_clean) %in% rna_cells   ]
# PLOT OUTLINE FRiP after improved plot
#limits <- range(c(A,B))
breaks <- seq(-0.01, 1.01, length.out=200)
...
c.keep <- eD.out_multi$Row.names %in% rna_cells
union_cells = union(eD_cells, rna_cells)
...
breaks <- seq(2, 11, length.out=200)
