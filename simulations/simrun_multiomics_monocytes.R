...
opath <- paste0("data/output/sim_monocytes/", current_date, "/results-sim")
ppath <- paste0("data/output/sim_monocytes/", current_date, "/pics-sim")
...
for (fname in ALLFILES) { 
  ...
  for (g1 in c(2000, 5000)){ 
    for (g2 in c(2000)) {
      ...
      for (it in 1:1) { 
        print(it)
        out <- SIMFUN_multiome_no_scrambling_bigger_empties_mono(raw.mat, raw.mat_atac, group1=g1, group2=g2, reorder.rate=0.1, down.rate.rna=0.20, down.rate.atac=0.20)  
