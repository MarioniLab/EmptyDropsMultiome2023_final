...
for (i in seq_along(eD_out) ) { 
  ...
  ffile <- file.path(opath, paste0(stub, "_fig2_monocytes.pdf"))  
  ...
  eD.out_multi$identity[ eD.out_multi$FDR <= 0.001 & !is.na(eD.out_multi$FDR) ] = "pass"
  ...
  emp.roc <- createRocPts(log(eD.out_multi$FDR+1e-3), eD.out_multi$identity_gt)
  emp_rna.roc <- createRocPts(log(eD.out_multi$FDR_RNA+1e-3), eD.out_multi$identity_gt)
  #emp_atac.roc <- createRocPts(log(eD.out_multi$FDR_chromatin+2e-3), eD.out_multi$identity_gt)
...
#   emp_atac.fdr = emp_atac.roc[,"empty"]/rowSums(emp_atac.roc)
#   emp_atac.tp1 = emp_atac.roc[,"large cells"]/g1
#   emp_atac.tp2 = emp_atac.roc[,"small cells"]/g2
  ...
  observations2d = data.frame(   
    ...
    "identity" = c( rep("eD (large cells) ", length_rep), rep("eD (small cells)", length_rep), 
                    ...
                    #rep("eD-atac (large cells) ", length_rep), rep("eD-atac (small cells)", length_rep) 
                  )
    )
